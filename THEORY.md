...
This is exact: it comes from differentiating the softmax weights, whose Jacobian is $\partial\alpha/\partial q = s\,(\text{diag}(\alpha) - \alpha\alpha^\top)K$. The rank-1 term $A(a)\otimes\bar{k}$ is the off-diagonal (mean-field / softmax-covariance) coupling; keeping it is what makes the expression the *exact* Jacobian rather than the naive uncentered form $s\sum_i\alpha_i(v_i\otimes k_i)$. Note that "exact" and "numerically load-bearing" are different claims: an ablation that keeps the uncentered second moment but drops the rank-1 term (holding the anchor and softmax weights fixed) inflates the first-order relative error only marginally on diffuse, near-isotropic query clouds — the second moment already captures most of the local sensitivity there. The rank-1 term is retained because it is analytically correct and free to compute (both factors $A(a)$ and $\bar{k}$ are already formed for the fused apply, §4.5); whether it contributes materially is expected to depend on the input distribution and is an empirical question on structured/trained activations. The implementation and the causal cumsum derivation (§6.4) both use this exact form.
